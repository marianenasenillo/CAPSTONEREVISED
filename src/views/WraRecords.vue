...
const batchDelete = async () => {
  ...
  try {
    const { error } = await supabase.from('wra_records').delete().in('id', selectedIds.value)
...
watch(selectedYear, () => fetchWraRecords())
...
const deleteRecord = async (record) => {
  if (!confirm(`Are you sure you want to remove the WRA record for "${record.firstname} ${record.lastname}"? This action cannot be undone.`)) {
    return
  }
    ...
    <div class="service-page">
      ...
      <div class="hs-toolbar">
        ...
        <div class="hs-toolbar-right">
          ...
          <button v-if="userRole === 'Admin'" class="hs-btn hs-btn-secondary" @click="$router.push('/wraarchived')"><span class="mdi mdi-archive-outline"></span> View Archived</button>
        </div>
      </div>
        ...
        <div class="hs-table-scroll">
          <table class="hs-table">
            ...
            <tbody>
              ...
              <tr v-if="filteredRecords.length === 0"><td colspan="21" class="hs-table-empty">No records found.</td></tr>
            </tbody>
          </table>
        </div>
