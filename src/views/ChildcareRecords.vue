...
const batchDelete = async () => {
  ...
  try {
    const { error } = await supabase.from('childcare_vitamina_records').delete().in('id', selectedIds.value)
...
watch(selectedYear, () => fetchVitaminARecords())
...
const deleteRecord = async (record) => {
  if (!confirm(`Are you sure you want to remove the Vitamin A record for "${record.firstname} ${record.lastname}"? This action cannot be undone.`)) {
    return
  }
...
const exportCsv = () => {
  const headers = ['Purok','Surname','First Name','Mother Name','Sex','Birthday','Age in Years']
  const rows = filteredRecords.value.map(r => [r.purok, r.surname, r.firstname, r.mother_name, r.sex, r.birthday, r.age])
  downloadCsv(headers, rows, 'childcare_records.csv')
}
    ...
    <div class="service-page">
      ...
      <div class="hs-toolbar">
        ...
        <div class="hs-toolbar-right">
          ...
          <button v-if="userRole === 'Admin'" class="hs-btn hs-btn-secondary" @click="$router.push('/childcarearchived')"><span class="mdi mdi-archive-outline"></span> View Archived</button>
        </div>
      </div>
        ...
        <div class="hs-table-scroll">
          <table class="hs-table">
            ...
            <tbody>
              ...
              <tr v-if="filteredRecords.length === 0"><td colspan="11" class="hs-table-empty">No records found.</td></tr>
            </tbody>
          </table>
        </div>
