...
const selectAllChecked = computed({
  get: () => paginatedData.value.length > 0 && paginatedData.value.every(r => selectedIds.value.includes(r.id)),
  set: (val) => {
    if (val) {
      const ids = paginatedData.value.map(r => r.id)
      selectedIds.value = [...new Set([...selectedIds.value, ...ids])]
    } else {
      const pageIds = new Set(paginatedData.value.map(r => r.id))
...
const batchDelete = async () => {
  ...
  try {
    const { error } = await supabase.from('cervical_screening_records').delete().in('id', selectedIds.value)
...
watch(selectedYear, () => fetchCervicalRecords())
...
const filteredRecords = computed(() => {
  ...
  if (selectedScreened.value) {
    records = records.filter(r => String(r.screened).toLowerCase() === selectedScreened.value.toLowerCase())
  }
...
watch([searchQuery, selectedPurok, selectedScreened, selectedAgeRange], () => resetPage())
...
const exportCsv = () => {
  const headers = ['Purok','Last Name','First Name','Middle Name','Suffix','Age','Birthdate','Screened']
  const rows = filteredRecords.value.map(r => [r.purok, r.lastname, r.firstname, r.middlename, r.suffix, r.age, r.birthdate, r.screened])
  downloadCsv(headers, rows, 'cervical_screening_records.csv')
}
...
<template>
    <div class="service-page">
      <div class="hs-page-header">
        <div class="hs-breadcrumb">Dashboard / Maternal Care / CCS Records</div>
        <h1>Cervical Cancer Screening Records <span class="hs-badge hs-badge-info">As of {{ selectedYear }}</span></h1>
        <p class="hs-module-desc">View, edit, and manage cervical cancer screening records.</p>
      </div>
      ...
      <div class="hs-toolbar">
        <div class="hs-toolbar-left">
          ...
          <select v-model="selectedYear" class="hs-select hs-w-auto">
          ...
          <select v-model="selectedScreened" class="hs-select hs-w-auto">
      ...
      <div v-if="showEditModal" class="hs-modal-overlay" @click.self="showEditModal = false">
        <div class="hs-modal hs-modal-lg">
          <div class="hs-modal-header">
            <h3>Edit Cervical Record: {{ editRecord.firstname }} {{ editRecord.lastname }}</h3>
            ...
            <form @submit.prevent="saveEdit">
              ...
              <div class="hs-form-row">
                ...
                <div class="hs-form-group">
                  ...
                  <select v-model="editRecord.screened" class="hs-select">
                    <option value="Yes">Yes</option>
                    <option value="No">No</option>
                  </select>
                </div>
