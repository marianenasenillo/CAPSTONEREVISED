...
const batchDelete = async () => {
  ...
  try {
    const { error } = await supabase.from('deworming_records').delete().in('id', selectedIds.value)
...
watch(selectedYear, () => fetchDewormingRecords())
...
const deleteRecord = async (record) => {
  if (!confirm(`Are you sure you want to remove the deworming record for "${record.firstname} ${record.lastname}"? This action cannot be undone.`)) {
    return
  }
    ...
    <div class="service-page">
      ...
      <div class="hs-toolbar">
        ...
        <div class="hs-toolbar-right">
          ...
          <button v-if="userRole === 'Admin'" class="hs-btn hs-btn-secondary" @click="$router.push('/preventivehealtharchived')"><span class="mdi mdi-archive-outline"></span> View Archived</button>
        </div>
      </div>
